...
if ($total_reg > 0) {
	...
	<th>Status</th>	 	
	<th class="sortable-th" data-column="hora" data-type="time">Hora <i class="fas fa-sort"></i></th>		
	<th>Ações</th>
	</tr> 
	</thead> 
	<tbody>	
...
<td data-order="{$nome_cliente}"><sapn style="color:{$classe_alerta}"><i class="fa fa-square"></i></sapn> <b>Pedido ({$pedido})</b> / {$nome_cliente} <span class="{$classe_entrega}"><small>({$entrega}) </small></span>
...
<td data-order="{$valor}">R$ {$valorF}</td>
<td class="{$classe_pago}">R$ {$total_pagoF} <small>{$texto_pago}</small></td>
<td>R$ {$trocoF}</td>
<td data-order="{$tipo_pgto}">{$tipo_pgto}</td>
<td>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#tabela').DataTable({
			...
			"order": [
				[1, "asc"]
			], // Ordena por cliente por padrão
			"columnDefs": [{
					"orderable": false,
					"targets": [0, 8]
				}, // Checkbox e ações não ordenáveis
				{
					"type": "string",
					"targets": 1
				}, // Cliente
			...
			$.ajax({
				...
				success: function(mensagem) {
					...
					if (msg == "Alterado com Sucesso") {
						...
						atualizarTabelaPedidos(); // refresh granular, mantém modal aberto
...
<script type="text/javascript">
	// Remove qualquer setTimeout/setInterval de reload
	// Adiciona refresh AJAX simples do tbody da tabela
	$(document).ready(function() {
		function atualizarTabelaPedidos() {
			$.ajax({
				url: 'paginas/pedidos/listar.php',
				type: 'POST',
				data: {
					status: $('#filtroStatus').val()
				}, // adapte se houver filtro
				success: function(data) {
					// Extrai apenas o <tbody> do HTML retornado
					var tbody = $(data).find('#tabela tbody').html();
					if (tbody) {
						$('#tabela tbody').html(tbody);
					}
					// Reaplica eventos dos checkboxes
					toggleBtnExcluirSelecionados();
				}
			});
		}
		setInterval(atualizarTabelaPedidos, 10000); // 10 segundos
	});
</script>

<style>
	/* Corrige fundo do cabeçalho ordenado mantendo padrão escuro */
	.thead-dark-mode th.sorting_asc,
	.thead-dark-mode th.sorting_desc,
	.thead-dark-mode th.sorting {
